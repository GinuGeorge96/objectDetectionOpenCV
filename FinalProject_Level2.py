    ...
    if grabResult.GrabSucceeded():
        ...
        if len(corners) > 0:
            ...
            for contour in contours:
                ...
                (x, y), (w, h), angle = rect              
                # Get Width and Height of the Objects by applying the Ratio pixel to mm
                ...
                if (24 <= object_height <= 26 and 7 <= object_width <= 9):
                    # Display rectangle
                ...
                if (31 <= object_height <= 33 and 9 <= object_width <= 11):
                     # Display rectangle
                    box = cv2.boxPoints(rect)
                    box = np.int0(box)
                    cv2.circle(img, (int(x), int(y)), 5, (0, 0, 255), -1)
                ...
                if (44 <= object_height <= 46 and 11 <= object_width <= 13):
                     # Display rectangle
                    box = cv2.boxPoints(rect)
                    box = np.int0(box)
                    cv2.circle(img, (int(x), int(y)), 5, (0, 0, 255), -1)
